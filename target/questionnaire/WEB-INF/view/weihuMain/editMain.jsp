				...
				<c:forEach items="${requestScope.map.main.questionList }" var="question" varStatus="vs">
					<tr class="info">
						<th >
							<div class="row">
								<div class="col-xs-6">
									<h4 style="padding-left: 40px">Question ${vs.index+1 }：${question.quesTitle }</h4>
								</div>
					...
					<tr class="success">
						<c:choose>
							<c:when test="${question.quesType eq 'radio'||question.quesType eq 'check'||question.quesType eq 'score'}">
								<c:choose>
							...
							<c:when test="${question.quesType eq 'text'}">
								<td style="padding-left: 40px"><%//答案 %>
									<label>
										<input type="text" name="${question.quesId }" >
									</label>
								</td>
							</c:when>
							<c:when test="${question.quesType eq 'texts'}">
							<td style="padding-left: 40px"><%//答案 %>
								<label>
									<textarea name="${question.quesId }"></textarea>
								</label>
								</td>
							</c:when>
							<c:when test="${question.quesType eq 'number'}">
							<td style="padding-left: 40px"><%//答案 %>
								<label>
									<input type="number" name="${question.quesId }" >
								</label>
							</td>
							</c:when>
							<c:when test="${question.quesType eq 'decimal'}">
							<td style="padding-left: 40px"><%//答案 %>
								<label>
									<input type="number" step="0.01" name="${question.quesId }" >
								</label>
							</td>
							</c:when>
						</c:choose>
					...
					<form id="editAnswer" action="<%=basePath %>question/addQuestionAndAnswer" method="post"  >
						...
						<div class="form-group">
							...
							<select class="form-control" name="questionType" id="questionType">
								...
								<option value="texts">文本(多行)</option>
			...
			<div class="modal-content">
				...
				<div class="modal-body" id="editModalBodyxg">
					<form id="editAnswerxg" action="<%=basePath %>question/editQuestion" method="post"  >
						...
						<div class="form-group">
							<label>问题:</label>
							<input type="text" class="form-control" id="questionTitlexg" name="questionTitle" id="questionTitlexg" />
						</div>
						<div class="form-group">
							<label for="questionTypexg">问题类型：</label>
				...
				<div class="modal-footer">
					<button class="btn btn-default" data-dismiss="modal">关闭</button>
					<button class="btn btn-primary" type="button" onclick="submitFormByIdxg('editAnswerxg')">确定</button>
				</div>
			</div>
		</div>
	</div>
