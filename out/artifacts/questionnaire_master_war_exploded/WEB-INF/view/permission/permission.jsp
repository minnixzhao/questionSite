    ...
    <script type="text/javascript">
        var setting = {
            async: {
                ...
                simpleData: {
                    enable:true,
                    idKey: "permissId", //节点数据中保存唯一标识的属性名称 默认值："id"
                    pIdKey: "pId",//节点数据中保存其父节点唯一标识的属性名称.enable = true 时生效，默认值："pId"
                    rootPId: 0 //用于修正根节点父节点数据，即 pIdKey 指定的属性值，enable = true 时生效，默认值：null
                },
                key: {
                    name: "permissDes" //zTree 节点数据保存节点名称的属性名称。默认值："name"
                }
            },
            callback: {
                beforeClick: function(treeId, node) {//用于捕获单击节点之前的事件回调函数，并且根据返回值确定是否允许单击操作默认值：null
                    var zTree = $.fn.zTree.getZTreeObj("tree");
                    $("#testIframe").attr("src","<%=basePath%>permission/updatePermissionView?permissId="+node.permissId+"&pId="+node.pId+"&permissName="+node.permissName+"&permissDes="+node.permissDes);
                    return true;
                },
        ...
        function addTreeNode(){
            if (zTree.getSelectedNodes()[0]) {
                ...
                $("#testIframe").attr("src","<%=basePath%>permission/addPermissionView?permissId="+node.permissId+"&pId="+node.permissId+"&permissName="+node.permissName+"&permissDes="+node.permissDes);
            } else {
        ...
        function removeTreeNode(){
            ...
            $.ajax({
                url:"<%=basePath%>permission/deletePermission/"+node.permissId,
                type:'DELETE',
